...
static std::wstring str2wstr(const std::string& string)
{
    if (string.empty())
    {
        return L"";
...
// Since codecvt is now deprecated API and no replacement is provided, we write our own.
static std::string wstr2str(const std::wstring& wide_string)
{
...
std::wstring datetime::towstring()
{
    return str2wstr(tostring());
}
...
void datetime::fromwstring(const std::wstring datetimestring)
{
    fromstring(wstr2str(datetimestring));
}

void datetime::fromwstring(const std::wstring datetimestring, funcfromwstring func)
{
    if (func != nullptr)
    {
        ymdhns result;
